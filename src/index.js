...
const stop = (state, action) => {
  const hasError = action.payload instanceof Error;
  return {
    ...state,
    isFetching: false,
    error: hasError ? action.payload : defaultStructState.error,
    data: hasError ? state.data : action.payload || defaultStructState.data,
  };
};
...
export const getStruct = (structId, getState = getStructState) => state =>
  getState(state)[structId] || defaultStructState;
